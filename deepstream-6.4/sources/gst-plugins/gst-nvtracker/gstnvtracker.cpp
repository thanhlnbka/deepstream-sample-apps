...
gst_nv_tracker_class_init (GstNvTrackerClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_SUB_BATCHES,
      g_param_spec_string ("sub-batches", "List of sub batces to be processed parallelly",
        "List of sub-batches (seperated by ;). Each sub-batch contains \
        list of source ids in that batch (seperated by ,) ",
        DEFAULT_SUB_BATCHES,
        (GParamFlags) (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS )));
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_LL_CONFIG_PATH:
    {
      ...
      // Populate trackerConfigFile and trackerConfigFilePerSubBatch
      ...
      nvtracker->trackerConfig.trackerConfigFile = g_strdup(singleConfigFile.c_str());
  ...
  switch (prop_id) {
    ...
    case PROP_SUB_BATCHES:
    {
      ...
      for (int32_t it = 0; it < (int32_t)subBatchesConfig.size(); it++)
      {
        ...
        for(int32_t i =0; i < ((int32_t)subBatch.size() - 1); i++)
        {
