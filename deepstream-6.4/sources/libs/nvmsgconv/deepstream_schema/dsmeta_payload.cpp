...
gchar*
ds3d_append_pointcloud(NvDsPayloadPriv* privObj, GuardDataMap datamap, gchar* msg)
{
    std::string message(msg ? msg : "");
    ds3dmsg::LidarPointCloud pc;
    uint32_t numPointsPacked = 0;
    ...
    vec4f* inputLidarPoints = (vec4f*)lidarFrame->base();

    for (uint32_t p = 0; p < numPoints; p++) {
        float w = ((privObj->datamapCfg.lidar_element_size == 4) ? inputLidarPoints->w : 1.0);
        if (inputLidarPoints->x || inputLidarPoints->y || inputLidarPoints->z) {
            ds3dmsg::Point* point = pc.add_points();
            point->set_x(inputLidarPoints->x);
            point->set_y(inputLidarPoints->y);
            point->set_z(inputLidarPoints->z);
            point->set_intensity(w);
            numPointsPacked++;
            if (numPointsPacked == privObj->datamapCfg.lidar_element_max_points) {
                break;
            }
        }
    }
...
generate_dsmeta_message_ds3d(void* privData, void* ptrDataMap, gboolean addLidarData, size_t& message_len)
{
    ...
    if (addLidarData) {
        message = ds3d_append_pointcloud(privObj, inputData, message);
    }
