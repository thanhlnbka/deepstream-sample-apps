...
gst_ds_nvmultiurisrc_bin_init (GstDsNvMultiUriBin * nvmultiurisrcbin)
{
  ...
  nvmultiurisrcbin->config->sock_fd = DEFAULT_SOCK_FD;
...
static int UnixSocketConnect(const char *socket_name) {
  int sock_fd = -1;
  ...
  while (connect(sock_fd, (const struct sockaddr *)&sock_addr,
    ...
    /* wait loop runs 10 times after sleep of 100ms each */
    if (wait_loop < 10) {
...
s_stream_api_impl (NvDsServerStreamInfo * stream_info, void *ctx)
{
  ...
  if (stream_info->uri.find ("/api/v1/") != std::string::npos) {
    ...
    if (g_strrstr (stream_info->value_change.c_str (), "add")
      ...
      gboolean is_ipc = nvmultiurisrcbin->config->uri
        && g_str_has_prefix (nvmultiurisrcbin->config->uri, "ipc://");
      if (is_ipc) {
          const char *prefix = "ipc://";
          const char *socket_path = nvmultiurisrcbin->config->uri + strlen(prefix);
          int sock_fd = UnixSocketConnect(socket_path);
          if (sock_fd == -1) {
            GST_WARNING_OBJECT (nvmultiurisrcbin,
                "Failed to add sensor id=[%s]; Connection Failed\n",
            stream_info->value_camera_id.c_str ());
            stream_info->status = STREAM_ADD_FAIL;
            stream_info->stream_log = "STREAM_ADD_FAIL, Connection failed, unable to add stream";
            stream_info->err_info.code = StatusInternalServerError;
            g_mutex_unlock (&nvmultiurisrcbin->bin_lock);
            return;
          }
          g_print("IPC socket path = %s and socket fd = %d\n", socket_path, sock_fd);
          nvmultiurisrcbin->config->sock_fd = sock_fd;
      }
      gboolean ret =
          gst_nvmultiurisrcbincreator_add_source (nvmultiurisrcbin->
          nvmultiurisrcbinCreator, nvmultiurisrcbin->config);
...
gGstNvMultiUriSrcBinStaticInit ()
{
  return gst_plugin_register_static (GST_VERSION_MAJOR, GST_VERSION_MINOR,
      ...
      DESCRIPTION, plugin_init_2, "7.0", LICENSE, BINARY_PACKAGE, PACKAGE,
      URL);
}
#endif
