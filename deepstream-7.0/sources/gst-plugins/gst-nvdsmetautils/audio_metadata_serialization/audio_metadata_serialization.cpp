...
void deserialize_data (GstBuffer *buf)
{
    ...
    if(user_meta->base_meta.meta_type == NVDS_USER_CUSTOM_META)
    {
        ...
        for (l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next)
        {
            ...
            str_len = std::min(strlen(framemeta.class_label().c_str()), static_cast<size_t> (MAX_LABEL_SIZE - 1));
            ...
            for (j = 0; j < framemeta.classifier_meta_list_size(); j++)
            {
                ...
                for (i = 0; i < nvdscm.label_info_list_size(); i++)
                {
                    ...
                    str_len = std::min(strlen(nvdsli.result_label().c_str()), static_cast<size_t> (MAX_LABEL_SIZE - 1));
                    strncpy (nvdslabelinfometa->result_label, nvdsli.result_label().c_str(),str_len);
